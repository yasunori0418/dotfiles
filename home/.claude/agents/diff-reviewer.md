...
- **オプションレンズ**(指定時のみ適用): `security`(セキュリティ / OWASP Top 10)
- キーワード対応: 「設計」「可読性」→ `design`、「テスト」→ `test`、「セキュリティ」「security」→ `security`
- registry に無い観点(例: performance)は下記の適用プロトコルのフォールバックで扱う
...
- 各指摘に由来レンズを `[design]` / `[test]` / `[security]`(その他のレンズを適用した場合はそのレンズ名)で併記する。同一箇所に複数レンズが当たった場合は最も重い severity で 1 件に統合し、タグを併記する(例: `[design][security]`)
